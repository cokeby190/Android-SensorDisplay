...
public class SensorConstStop_para extends Activity implements OnClickListener, SensorEventListener, LocationListener{
	...
	private Button b_risk_l, b_risk_r;
	private Button b_risk_rapid, b_risk_swerve;
	...
	private void initialize() {
		...
		b_risk_l = (Button) findViewById(R.id.b_risk_l);
		b_risk_r = (Button) findViewById(R.id.b_risk_r);
		b_risk_rapid = (Button) findViewById(R.id.b_risk_rapid);
		b_risk_swerve = (Button) findViewById(R.id.b_risk_swerve);
    	...
    	b_risk_l.setOnClickListener(this);
    	b_risk_r.setOnClickListener(this);
    	b_risk_rapid.setOnClickListener(this);
    	b_risk_swerve.setOnClickListener(this);
...
//			vert_flag = true;
	...
	public void onClick(View view) {
		...
		case R.id.b_risk_l:
			if(start_log == true && end_log == true) {
				tag_save_log("RISK LEFT", "GroundTruth_Risk");
			}
			else
				cannotTag();
		...
		case R.id.b_risk_r:
			if(start_log == true && end_log == true) {
				tag_save_log("RISK RIGHT", "GroundTruth_Risk");
			}
			else
				cannotTag();
		...
		case R.id.b_risk_rapid:
			if(start_log == true && end_log == true) {
				tag_save_log("RAPID ACCELERATION", "GroundTruth_Risk");
			}
			else
				cannotTag();
		...
		case R.id.b_risk_swerve:
			if(start_log == true && end_log == true) {
				tag_save_log("SWERVE", "GroundTruth_Risk");
			}
			else
				cannotTag();
				...
				if(check_acceleration <= gravity) {
					...
					if(gps_speed == 0.0) {
						if(EventState.checkTransit(State.STOP)) {
							processStateTime(System.currentTimeMillis() - EventState.getStartTs(), State.STOP);
					...
					if(flat_flag == true) {
						...
						if(fwd_acc <= (back_thres*-1)) {
							if(EventState.checkTransit(State.DEC)) {
								processStateTime(System.currentTimeMillis() - EventState.getStartTs(), State.DEC);
							...
							if(EventState.checkTransit(State.ACC)) {
								processStateTime(System.currentTimeMillis() - EventState.getStartTs(), State.ACC);
...
//								aggr_save_log("RAPID ACCELERATION/DECELERATION");
								...
								tv_event.setText("RAPID ACCELERATION/DECELERATION");
					...
					if((angle_z - prev_z) > 1.0) {
						if(EventState.checkDir(State.LEFT)) {
							if(EventState.getState() == State.ACC && rel_acc >= acc_turn_aggr) {
								...
								tv_event.setText("RISKY LEFT TURN");
...
//								new show_warning().execute();
							...
							processStateTime(System.currentTimeMillis() - EventState.getStartTs(), State.LEFT);
						...
						if(EventState.checkDir(State.RIGHT)) {
							...
							if(EventState.getState() == State.ACC && rel_acc >= acc_turn_aggr) {
								...
								tv_event.setText("RISKY RIGHT TURN");
...
//								new show_warning().execute();
							...
							processStateTime(System.currentTimeMillis() - EventState.getStartTs(), State.RIGHT);
...
//			}
			...
			if(start_log == true && end_log == true) {
				...
				//save to SD
				data_save += time_stamp("time") + "\t" + msg;
//				data_save += time_stamp("time") + "\t" + q_state.get(0) + "\n";
			...
			if(start_log == true && end_log == true) {
				...
				//save to SD
				data_save += time_stamp("time") + "\t" + msg;
//				data_save += time_stamp("time") + "\t" + q_state.get(q_state.size()-1) + "\n";
	...
	private void processStateTime(long time, State state) {
...
//				new show_warning().execute();
			...
			if(start_log == true && end_log == true) {
				//save to SD
				data_save += "\t" + convert + "\n";
//				data_save += time_stamp("time") + "\t" + q_state.get(0) + "\t" + convert + "\n";
				save_ext.writeExt(curr_time , data_save, "Eventlog");
				
				data_save = "";
			}
		} //IF TIME LIST IS NOT EMPTY, AND LIST SIZE IS 1 LESSER THAN STATELIST-------------------------------------------------//
		else if(!q_time.isEmpty() && q_time.size() == q_state.size()-1 && q_state.get(q_state.size()-1) != state) {
			q_time.add(time);
			if(convert< 0.5) {
				aggressive = " AGGRESSIVE";
				if(q_state.get(q_state.size()-1) != State.CONST)
